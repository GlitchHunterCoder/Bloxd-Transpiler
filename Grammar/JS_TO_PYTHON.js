var JS_TO_PYTHON = {
  ...
  rules: {
    program: {
      ...
      into: function(caps){ return caps.stmts.filter(function(s){ return s.trim(); }).join("\n"); }
    },
    statement: {
      variants: [
    ...
    funcDecl: {
      pattern: [
        "KW_FUNCTION", { token:"IDENT", name:"name" },
      ...
      into: function(caps, ctx) {
        ...
        return i + "def " + caps.name + "(" + (caps.params||"") + "):\n" +
    ...
    elseClause: {
      variants: [
        {
          pattern: ["KW_ELSE", { rule:"ifStmt", name:"s" }],
          into: function(caps){ return caps.s.replace(/^(\s*)if /, "$1elif "); }
        },
        {
          pattern: [
            "KW_ELSE", "LBRACE",
    ...
    whileStmt: {
      pattern: [
        "KW_WHILE", "LPAREN", { rule:"expr", name:"cond" }, "RPAREN",
        "LBRACE",
        { action: function(ctx){ ctx.set("indent", ctx.get("indent")+1); } },
        { many:"statement", name:"body" },
        { action: function(ctx){ ctx.pop("indent"); } },
    ...
    returnStmt: {
      pattern: ["KW_RETURN", { opt:"expr", name:"val" }],
      into: function(caps, ctx){ return ctx.indent() + "return" + (caps.val ? " "+caps.val : ""); }
    },
    printStmt: {
      pattern: ["KW_CONSOLE_LOG", "LPAREN", { opt:"argList", name:"args" }, "RPAREN"],
      into: function(caps, ctx){ return ctx.indent() + "print(" + (caps.args||"") + ")"; }
    },
    varDecl: {
      variants: [
        {
          pattern: ["KW_VAR", { token:"IDENT", name:"n" }, "ASSIGN", { rule:"expr", name:"v" }],
          into: function(caps, ctx){ return ctx.indent() + caps.n + " = " + caps.v; }
        },
        {
          pattern: ["KW_VAR", { token:"IDENT", name:"n" }],
          into: function(caps, ctx){ return ctx.indent() + caps.n + " = None"; }
        }
      ]
    },
    exprStmt: {
    ...
    unaryExpr: {
      variants: [
        ...
        { pattern: ["NOT",   { rule:"callExpr", name:"e" }], into: function(caps){ return "not "+caps.e; } },
      ...
      variants: [
        {
          pattern: [
            ...
            "LPAREN", { opt:"argList", name:"args" }, "RPAREN",
            { many:"callSuffix", name:"s" }
          ],
    ...
    primary: {
      variants: [
        ...
        { pattern: [{ token:"IDENT", name:"n" }, "ASSIGN", { rule:"expr", name:"v" }], into: "$n = $v" },
